...
april_tag_map = [[1.5,0,3*np.pi/2,2],
                 [4.0,0,3*np.pi/2,1],
                 [6.5,0,3*np.pi/2,2],
                 [8.0,1.5,0,1],
                 [8.0,4.0,0,2],
...
def tag_callback(msg):
    ...
    if msg.ids:
        ...
        # Get position of robot from april tags
        ...
        for j,v in enumerate(april_tag_map):
            april_tag_distance = np.sqrt((v[0]-april_tag[0])**2 + (v[1]-april_tag[1])**2)
        ...
        if april_tag_min_distance < 0.7 and april_tag_map[j][3] == msg.ids[x_argmin]:
            ...
            robot_pos[0] = april_tag_map[april_tag_min_index][0] - camera_pos[2]*np.cos(april_tag_map[april_tag_min_index][2]) - camera_pos[0]*np.sin(april_tag_map[april_tag_min_index][2])
            robot_pos[1] = april_tag_map[april_tag_min_index][1] - camera_pos[2]*np.sin(april_tag_map[april_tag_min_index][2]) + camera_pos[0]*np.cos(april_tag_map[april_tag_min_index][2])

            # Retrive orientation where 0 is looking at april tag, positive is looking to the left of april tag
            robot_pos[2] = april_tag_map[april_tag_min_index][2] * matrix[2] * (np.pi/2)
